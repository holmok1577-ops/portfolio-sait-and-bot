...
- 🧭 Эскалация при отсутствии ответа: уточнение, повторное предупреждение, затем сбор контактов для консультации
...
- **AI**: OpenAI API (ProxyAPI), ChromaDB
...
База знаний проекта находится в `data/rag_documents/`. В текущей версии подготовлен 21 Markdown-документ: профиль, услуги, проекты GitHub, FAQ, общие материалы про AI/RAG/промпт-инжиниринг, метрики MVP и политика эскалации. Имена файлов используются в ASCII, чтобы избежать проблем с кодировкой на сервере.
...
- Секреты хранятся в `.env`, файл не должен попадать в Git

## Эскалация в ассистенте

Если RAG не находит релевантный ответ в базе знаний, ассистент не должен выдумывать информацию:

1. Первый неизвестный вопрос — просит уточнить формулировку.
2. Второй неизвестный вопрос подряд — сообщает, что тема может быть вне базы знаний или компетенций.
3. Третий неизвестный вопрос подряд — предлагает оставить имя, телефон и email для консультации.
4. После получения контактов заявка сохраняется в БД и отправляется администратору в Telegram.
