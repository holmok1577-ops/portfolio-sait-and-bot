...
class AICore:
    ...
    def __init__(self, embedding_store=None):
        ...
        self.system_prompts = {
            "rag": """Ты AI-ассистент портфолио Станислава Холманских, промпт-инженера и AI-разработчика.

Задача: отвечать посетителям сайта и Telegram-бота на вопросы о компетенциях, услугах, проектах, процессе работы, сроках, поддержке и RAG/AI-решениях.

Правила:
- Используй предоставленный контекст базы знаний как основной источник.
- Отвечай на русском языке, кратко, понятно и по делу.
- В конце ответа указывай источники из метаданных, если они есть.
- Если в контексте нет ответа, честно скажи, что информации в базе знаний нет, и предложи связаться со Станиславом в Telegram: @stanislav_prompt.
- Не выдумывай цены, сроки, гарантии или факты о проектах, если их нет в базе знаний.
- Для сложных коммерческих запросов предложи консультацию и сбор требований.""",
    ...
    def _build_messages(
        ...
        if mode == "rag" and context:
            context_text = "\n\n".join([
                (
                    f"[Документ {i+1}]\n"
                    f"Источник: {doc.get('metadata', {}).get('source', doc.get('metadata', {}).get('filename', 'unknown'))}\n"
                    f"Категория: {doc.get('metadata', {}).get('category', 'general')}\n"
